...
public abstract class MenuFunction extends Data_dao {
  ...
  // 게시글 마지막 넘버 확인
  // 추가 수정 ArrayList로 받으면 .lastIndexOf 사용
  ...
  public ArrayList<Board> readALLBoard() {
    ...
    try {
      ...
      for (Board board : boardlist) {
        System.out.printf("%-6d%-12s%tY-%tm-%-6td%-10s%n", board.getBno(), board.getBwriter(),
            board.getBdate(), board.getBdate(), board.getBdate(), board.getBtitle());
      }
      rs.close();
      pstmt.close();
    } catch (SQLException e) {
  ...
  public void readDB() {
    ...
    int num = 0;
    while (true) {
      if (isNumberic(bnoS) && Integer.parseInt(bnoS) <= boardLastNum()) {
        num = Integer.parseInt(bnoS);
        break;
      } else {
        System.out.println("다시 입력해주세요.");
        bnoS = sc.nextLine();
